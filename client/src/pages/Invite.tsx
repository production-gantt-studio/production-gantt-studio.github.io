 ...
 * 以前は閲覧者だけログインせずプレビュー画面へ飛ばしていたが、進捗担当は
...
export default function Invite() {
  ...
  const heading = isProgressMember ? "進捗担当として招待されています" : "編集者として招待されています";
  const description = isProgressMember
    ? "招待されたメールアドレスでログインすると、この案件のタスクの「ステータス」と「担当者」を変更できます。タスクの追加・削除、日程の変更、メンバーの招待はできません。"
    : "招待されたメールアドレスでログインすると、タスクの追加・削除、日程変更、メンバーの招待まで、この案件をすべて編集できます。";
