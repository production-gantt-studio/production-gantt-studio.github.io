...
export default function Home() {
    ...
    if ((!readOnly || progressOnly) && !blankPreview) {
      ...
      if (isAuthenticated && projectId && remoteProjectQuery.data?.project) {
        const timer = window.setTimeout(() => {
          const payload = { publicId: projectId, title: nextProject.title, client: nextProject.client, eventMonth: nextProject.eventMonth ?? null, data: JSON.stringify(nextProject) };
          // 保存に失敗したことに気づけないのが一番困るので、必ず画面に出す。
          // 自動保存は入力のたびに走るため、同じidのトーストで上書きして重ねない。
          const onError = () => toast.error("変更を保存できませんでした。通信状況を確認し、画面を再読み込みしてください。", { id: "remote-save-failed" });
          if (progressOnly) remoteUpdateTaskProgress.mutate(payload, { onError });
          else remoteUpdateProject.mutate(payload, { onError });
        }, 450);
  ...
  const updateTask = (id: string, patch: Partial<Task>, cascade = false) => {
    ...
    if (readOnly) {
      const touchesOnlyProgressFields = Object.keys(patch).every((key) => key === "status" || key === "assignee");
      if (!progressOnly || !touchesOnlyProgressFields) return;
    }
  ...
  return (
    <div className={`studio-shell ${isNarrow ? "is-mobile-shell" : ""} ${linkOnlyView ? "shared-project-shell" : ""}`}>
      {!isNarrow && (
