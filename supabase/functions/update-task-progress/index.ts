// update-task-progress: 進捗担当(project_members.role = "viewer")でも通せる、
...
//     ので、進捗担当は今までどおり 403 で弾かれる(＝既存の編集者の動作に
//     手を入れずに済み、退行の危険が無い)。
...
//     検査漏れという失敗の形そのものが無い。
//   - 土台が常にDBの最新データなので、進捗担当の端末が古いままでも、
//     その間に編集者が加えた変更を巻き戻さない。
//
// 拒否ではなく無視にした理由: 画面側は既に進捗担当へ状態・担当者以外の操作を
...
// 開催月は進捗担当が変更できる項目ではないので、DBの値をそのまま残す。
