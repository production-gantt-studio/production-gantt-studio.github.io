...
| 初回管理者メール送信 | 送信処理PASS／受信HOLD | Supabase Authログは`mail.send`を記録したが、2026-08-22の実送信は受信確認が取れなかった |
...
公開クライアントは `/auth/confirm` の `token_hash` 検証方式を実装済みであり、GitHub Pages上でも同ルートが404にならず表示されることを確認しました。一回限りの検証リンクで、同ルートのログインと再読み込み後のセッション保持も実測済みです。編集者招待は、`create-invite` が対象メール専用の一回限り認証トークンと招待トークンを同時に発行し、GitHub Pagesの`/production-gantt-studio/auth/confirm`を経由して受諾するため、Supabaseの通常メールが届かなくても運用できます。リンクは既存の招待画面からコピーされ、メール作成画面に挿入されます。Supabase側の標準メールテンプレートは未変更で、2026-08-22の送信要求は`mail.send`ログに記録された一方、受信確認は取れませんでした。この未達は、既存管理者がセッションを失った際の通常ログイン経路だけに残る運用課題です。Supabase Freeの送信上限は毎時2通のため、通常メール送信を短時間に連続させないでください。
...
| 標準ログインメール送信 | HOLD | Supabase Authの`mail.send`記録は確認したが、実メールの受信確認は取れない。既存管理者のセッション再取得だけに残る課題 |
...
1. Supabase Authメールテンプレートをtoken_hash形式へ更新し、既存管理者がセッションを失った場合の通常ログインメールが受信トレイへ到達することを確認する。

上記以外の認証リンク検証、案件保存、編集者招待、共有・アーカイブ検証は、本番ブラウザ、API又はDBで完了しています。検証で発行した親子共有リンクはすべて取消済みです。短時間の再認証関数は利用後に無効化し、検証用編集者2件の案件権限も取消済みです。検証案件`Sample — 公開検証用`はアーカイブ状態で2026-09-21まで保持します。実在のクライアント名・案件名は使っていません。
