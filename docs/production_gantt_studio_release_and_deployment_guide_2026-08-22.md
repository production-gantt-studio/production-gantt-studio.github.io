...
| 領域 | 状態 | 確認済みの内容 |
|---|---|---|
| GitHub main | 公開反映済み | 最新コミット `cfbc977`。認証確認ルート、初回管理者修正、RLS補助関数の非公開化、Sample誤移行防止、編集者の新規案件ボタン非表示、メール不要の認証付き編集者招待を含む |
...
| 初回管理者メール送信 | 非採用 | Supabase Freeの既定メールは送信処理を記録したが受信確認が取れず、テンプレート編集にはカスタムSMTPが必要と本番画面で確認。追加契約・秘密情報を導入せず、編集者は認証付き招待リンクで運用する |
| 初回管理者の認証画面遷移 | PASS | 一回限り`token_hash`リンクで独立版へログインし、再読み込み後も管理者セッションが保持された |
| 共有・アーカイブ復元 | 実測PASS（API/DB） | 本番APIで親子共有の作成・親取消連動を確認。本番DBでアーカイブ→復元→再アーカイブを確認 |
| 編集者招待 | PASS | 別メールの認証付き招待リンクを作成し、Supabaseの通常メールを使わずにログイン・受諾・編集者表示まで確認。検証権限はテスト後に取消済み |
...
公開クライアントは `/auth/confirm` の `token_hash` 検証方式を実装済みであり、GitHub Pages上でも同ルートが404にならず表示されることを確認しました。一回限りの検証リンクで、同ルートのログインと再読み込み後のセッション保持も実測済みです。編集者招待は、`create-invite` が対象メール専用の一回限り認証トークンと招待トークンを同時に発行し、GitHub Pagesの`/production-gantt-studio/auth/confirm`を経由して受諾するため、Supabaseの通常メールが届かなくても運用できます。リンクは既存の招待画面からコピーされ、メール作成画面に挿入されます。2026-08-22にSupabase本番のメール設定を確認した結果、Freeの既定メールを使う状態ではテンプレート編集にカスタムSMTPが必要でした。既定メールは`mail.send`ログを記録した一方で受信確認が取れなかったため、追加契約・SMTP資格情報を入れず、通常ログインメールを運用経路から外します。管理者は現在のセッションを保持し、編集者は認証付き招待リンクで参加します。
...
| 標準ログインメール送信 | 非採用 | Freeの既定メールでは受信確認が取れず、テンプレート編集にはカスタムSMTPが必要。追加費用・資格情報を避けるため、運用経路から外す |
| token_hashログインとセッション保持 | PASS | 一回限り認証リンクから独立版へログインし、再読み込み後も管理者状態を保持 |
| 新規案件の保存 | PASS | 管理者画面から`Sample — 公開検証用`を作成し、「保存済み」表示と再読み込み後の保持を確認 |
| 編集者招待・受諾 | PASS | 編集者が発行したリンクに一回限り認証を埋め込み、別メールで認証・受諾後に対象案件で編集者表示を確認。リンクはGitHub Pagesの正しいサブパスへ戻ることを実測 |
| 編集者の新規案件制御 | PASS | 編集者には新規案件ボタンを表示しない修正を回帰テスト、型検査、本番ビルド、GitHub Pages公開で確認 |
...
Supabase Freeの既定メールは通常ログインに使いません。テンプレート編集にはカスタムSMTPが必要であり、追加費用やSMTP資格情報を導入しない方針のためです。編集者は管理者が発行する認証付き招待リンクで、メール内のURLを開くだけでログイン・参加できます。管理者は現在のログイン状態を維持して利用します。

認証リンク検証、案件保存、編集者招待、共有・アーカイブ検証は、本番ブラウザ、API又はDBで完了しています。検証で発行した親子共有リンクはすべて取消済みです。短時間の再認証関数は利用後に無効化し、検証用編集者2件の案件権限も取消済みです。検証案件`Sample — 公開検証用`はアーカイブ状態で2026-09-21まで保持します。実在のクライアント名・案件名は使っていません。
...
- [認証メール未達・Sample移行問題の実測記録](auth_delivery_and_sample_migration_incident_2026-08-22.md)
