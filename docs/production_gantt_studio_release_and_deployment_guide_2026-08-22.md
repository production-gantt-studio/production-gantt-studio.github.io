...
| 領域 | 状態 | 確認済みの内容 |
|---|---|---|
| GitHub main | 公開反映済み | 最新コミット `b4c870e`。認証確認ルート、初回管理者修正、RLS補助関数の非公開化、Sample誤移行防止、編集者の新規案件ボタン非表示を含む |
...
| 編集者招待 | PASS | 別メールの招待作成、編集者ログイン、招待受諾、本番`project_members`の`active/editor`状態を確認 |
...
公開クライアントは `/auth/confirm` の `token_hash` 検証方式を実装済みであり、GitHub Pages上でも同ルートが404にならず表示されることを確認しました。一回限りの検証リンクで、同ルートのログインと再読み込み後のセッション保持も実測済みです。Supabase側の標準メールテンプレートは共有ブラウザのセッション不一致により未変更で、旧PKCEの `/auth/callback` も併存して受け付けます。2026-08-22の送信要求はSupabase Authの`mail.send`ログに記録されましたが、受信確認は取れませんでした。従って、`{{ .SiteURL }}/auth/confirm?token_hash={{ .TokenHash }}&type=magiclink` へのテンプレート統一と、通常メールの受信到達性だけは未解決です。Supabase Freeの送信上限は毎時2通のため、実運用ではログイン・招待送信を短時間に連続させないでください。
...
| ログインメール送信 | HOLD | Supabase Authの`mail.send`記録は確認したが、実メールの受信確認は取れない |
...
| 編集者招待・受諾 | PASS | 別メールへ編集者招待を作成し、ログイン後の招待受諾画面で編集者表示、本番`project_members`で`active/editor`を確認 |
...
1. Supabase Authメールテンプレートをtoken_hash形式へ更新し、通常の認証メールが受信トレイへ到達することを確認する。

上記以外の認証リンク検証、案件保存、編集者招待、共有・アーカイブ検証は、本番ブラウザ、API又はDBで完了しています。検証で発行した親子共有リンクはすべて取消済みで、一回限り認証リンク関数は利用後にHTTP 410へ無効化し、テスト用トークンと一時ファイルを削除しました。検証案件`Sample — 公開検証用`はアーカイブ状態で2026-09-21まで保持します。実在のクライアント名・案件名は使っていません。
